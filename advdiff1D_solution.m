...
T   = T0 + dT .* exp(-(xc-W/2).^2/(4*wT^2));     % initialise T array at Tr
...
while t <= tend
    ...
    wTt = sqrt(wT^2 + k0*t);
    Ta  = T0 + dT .* wT./wTt .* exp(-(xc-W/2  -u0*t).^2/(4*wTt^2)) ...
             + dT .* wT./wTt .* exp(-(xc-W/2+W-u0*t).^2/(4*wTt^2)) ...
             + dT .* wT./wTt .* exp(-(xc-W/2-W-u0*t).^2/(4*wTt^2));
...
fmmm = f(ind(1:end-6));  % i-3
fmm  = f(ind(2:end-5));  % i-2
fm   = f(ind(3:end-4));  % i-1
fc   = f(ind(4:end-3));  % i
fp   = f(ind(5:end-2)); 
fpp  = f(ind(6:end-1));
fppp = f(ind(7:end-0));
